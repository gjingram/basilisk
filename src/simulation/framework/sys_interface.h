...
#include <vector>
#include <stdint.h>
#include "framework/sys_model_task.h"
...
struct MessageInterfaceMatch {
    ...
    std::string message_source;  //!< Name of process to get message from

    std::string message_dest;  //!< Name of process to put message in

    int64_t source;  //!< The ID for the buffer to copy messages from
                     //!< (negative indicates not ready)

    int64_t destination;  //!< The ID for the buffer to copy messages to
                          //!< (negative indicates not ready)

    uint64_t update_counter;  //!< The number of times a message had been
                              //!< written last time it was routed.
};
